<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否购房" prop="shifougoufang">
          <el-select :disabled="ro.shifougoufang" v-model="ruleForm.shifougoufang" placeholder="请选择是否购房">
            <el-option
                v-for="(item,index) in shifougoufangOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否购房" prop="shifougoufang">
	      <el-input v-model="ruleForm.shifougoufang"
                placeholder="是否购房" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="对他的期待" prop="duitadeqidai">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.duitadeqidai" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.duitadeqidai" label="对他的期待" prop="duitadeqidai">
                    <span v-html="ruleForm.duitadeqidai"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="自我介绍" prop="ziwojieshao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.ziwojieshao" label="自我介绍" prop="ziwojieshao">
                    <span v-html="ruleForm.ziwojieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.suozaichengshi = true;
          ...
          if(o=='shifougoufang'){
            this.ruleForm.shifougoufang = obj[o];
	    this.ro.shifougoufang = true;
            continue;
          }
          if(o=='duitadeqidai'){
            this.ruleForm.duitadeqidai = obj[o];
	    this.ro.duitadeqidai = true;
            continue;
          }
          if(o=='ziwojieshao'){
      ...
      this.$http({
		...
		if(json.xingbie!=''&&json.xingbie){
          ...
          this.$message.error(data.msg);
            ...
            this.shifougoufangOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xiaoyouxiangqin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.duitadeqidai = this.ruleForm.duitadeqidai.replace(reg,'../../../social/upload');
	this.ruleForm.ziwojieshao = this.ruleForm.ziwojieshao.replace(reg,'../../../social/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoyouxiangqin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoyouxiangqinCrossAddOrUpdateFlag = false;
						 this.parent.search();
