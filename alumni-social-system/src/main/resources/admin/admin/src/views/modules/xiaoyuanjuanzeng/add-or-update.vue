<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="年度" prop="niandu">
          <el-input v-model="ruleForm.niandu" 
              placeholder="年度" clearable  :readonly="ro.niandu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="年度" prop="niandu">
              <el-input v-model="ruleForm.niandu" 
                placeholder="年度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="项目" prop="xiangmu">
          <el-select :disabled="ro.xiangmu" v-model="ruleForm.xiangmu" placeholder="请选择项目">
            <el-option
                v-for="(item,index) in xiangmuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="捐赠用途" prop="juanzengyongtu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="捐赠用途"
                  v-model="ruleForm.juanzengyongtu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.juanzengyongtu" label="捐赠用途" prop="juanzengyongtu">
                    <span>{{ruleForm.juanzengyongtu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="捐赠详情" prop="juanzengxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.juanzengxiangqing" label="捐赠详情" prop="juanzengxiangqing">
                    <span v-html="ruleForm.juanzengxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='juanzengyongtu'){
            this.ruleForm.juanzengyongtu = obj[o];
	    this.ro.juanzengyongtu = true;
            continue;
          }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/xiangmu/xiangmu`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xiangmuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.juanzengxiangqing = this.ruleForm.juanzengxiangqing.replace(reg,'../../../social/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoyuanjuanzeng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoyuanjuanzengCrossAddOrUpdateFlag = false;
						 this.parent.search();
